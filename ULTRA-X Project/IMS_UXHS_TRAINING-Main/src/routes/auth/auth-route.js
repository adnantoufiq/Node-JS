...
const { insertUserQuery, getUserRow, getUser } = require("../../sql_query/user-insert-query");      // insert-query
const { jwtAuthenticate } = require("../../middlewares/auth-token/authenticate-token");
const { signupUserValidation } = require("../../middlewares/signup/middleware-signup");  //
const { loginUserValidation } = require("../../middlewares/login/middleware-login");
const { loginQuery } = require("../../sql_query/login-query");           // login-query
const { updatedUserBodyData } = require("../../middlewares/signup/update-user-middleware");
const { getUserBodyData } = require("../../middlewares/signup/update-user-validator")
...
authRoute.post("/login", loginUserValidation, async (req, res) => {
  ...
  try {
    ...
    if (result.length > 0) {
      ...
      const isPasswordCorrect = await bcrypt.compare(user.password, hashPassword);  //compare passwords
...
authRoute.put("/update/:id", jwtAuthenticate, getUserBodyData, updatedUserBodyData, async (req, res) => {
...
authRoute.get("/get-user", jwtAuthenticate, createPagination, async (req, res) => {
  const paginationData = req.body.paginationData;
  try {
    const [rows] = await pool.query(getUserRow);
    const [result] = await pool.query(getUser, [paginationData.itemsPerPage, paginationData.offset]);
    let totalRows = 0;
    if (rows.length <= 0) {
